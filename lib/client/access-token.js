...
module.exports = function(config) {
  ...
  //
  // * `callback` - The callback function returning the results.
  // An error object is passed as first argument and the blank result
  // as last.
  //
  
  function revoke(callback) {
		var params = { client_secret: config.clientSecret, client_id: config.clientID, token: this.token.access_token };
		params.token = this.token.access_token;
		
		core.api('POST', config.revokePath, params, function(error, result){
		  if (!error) { 
